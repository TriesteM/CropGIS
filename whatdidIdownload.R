...
whatdidIdownload=function(year=year,avhdir=avhdir,copyto=F){
    
    if (!require("pacman")) install.packages("pacman")
    pacman::p_load(data.table,lubridate)
    
    #figure out what months they belong to:
    question=list.files(paste(avhdir,year,sep=''),pattern='_tif')
    
    splits=strsplit(question,'_')
    integers=as.data.frame(do.call(cbind,splits),stringsAsFactors=FALSE)[3,]
